...
const DEFAULT_HR_ROLE_CODE = 'HR_SPECIALIST'
...
const visibleTemplates = computed(() =>
  templates.value.filter((template) => template.businessType !== 'PERFORMANCE_APPEAL'),
)
...
function normalizeHrRoleCode(value: string | null | undefined) {
  const trimmed = String(value ?? '').trim()
  return !trimmed || trimmed === 'HR' ? DEFAULT_HR_ROLE_CODE : trimmed
}

function newNode(): DraftNode {
  return { nodeType: 'DIRECT_MANAGER', userId: '', roleCode: DEFAULT_HR_ROLE_CODE }
}
...
async function loadData() {
  loading.value = true
  try {
    const [templateList, departmentTree] = await Promise.all([
      fetchWorkflowTemplates(),
      fetchDepartmentTree(),
    ])
...
function openEdit(template: WorkflowTemplate) {
  Object.assign(form, {
    ...
    nodes: template.nodes.map((node) => ({
      ...
      roleCode:
        node.nodeType === 'HR'
          ? normalizeHrRoleCode(String(node.approverRule.roleCode ?? ''))
          : DEFAULT_HR_ROLE_CODE,
    })),
  })
  dialogVisible.value = true
}
...
function toPayload(): WorkflowTemplatePayload | null {
  ...
  const nodes = form.nodes.map((node, index) => {
    ...
    if (node.nodeType === 'HR') approverRule.roleCode = normalizeHrRoleCode(node.roleCode)
  ...
  return {
    ...
    nodes: nodes as NonNullable<typeof nodes[number]>[],
...
<template>
  <PageFrame
    title="流程模板"
    description="配置业务审批链。提交后会冻结节点和已解析审批人，后续修改只影响新提交的单据。"
  >
    <template #actions>
    ...
    <el-table v-loading="loading" :data="visibleTemplates" class="data-table">
      ...
      <el-table-column label="适用范围" min-width="160">
        <template #default="{ row }">
          {{ row.departmentIds.length ? `部门 ${row.departmentIds.join('、')}` : '全公司' }}
        </template>
      </el-table-column>
      <el-table-column label="节点" min-width="260">
        <template #default="{ row }">
          <el-tag
            ...
            :key="node.nodeNo"
            effect="plain"
            class="workflow-node-tag"
          >
            {{ node.nodeNo }}. {{ nodeLabel(node.nodeType) }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="76" fixed="right">
        <template #default="{ row }">
          <el-tooltip content="编辑模板">
            <el-button text :icon="Edit" aria-label="编辑模板" @click="openEdit(row)" />
          </el-tooltip>
        </template>
      </el-table-column>
    </el-table>
    ...
    <EmptyState
      ...
      title="暂无流程模板"
      description="创建模板后，请假和后续业务将按范围、优先级和版本选择审批链。"
    />
      ...
      <el-form label-position="top" class="workflow-template-form">
        <div class="form-grid">
          <el-form-item label="模板编码">
            <el-input
              v-model="form.code"
              :disabled="isEditing"
              placeholder="例如 LEAVE_STANDARD"
            />
          </el-form-item>
          <el-form-item label="模板名称"><el-input v-model="form.name" /></el-form-item>
          <el-form-item label="业务类型">
            <el-select v-model="form.businessType" :disabled="isEditing">
              <el-option
                v-for="(label, value) in businessLabels"
                :key="value"
                :label="label"
                :value="value"
              />
          ...
          <el-form-item v-if="!isEditing" label="模板版本">
            <el-input-number v-model="form.templateVersion" :min="1" />
          </el-form-item>
        ...
        <div class="workflow-node-editor">
          <div class="workflow-node-heading">
            <strong>审批节点</strong>
            <el-tooltip content="新增节点">
              <el-button circle :icon="Plus" aria-label="新增节点" @click="addNode" />
            </el-tooltip>
          </div>
          <div v-for="(node, index) in form.nodes" :key="index" class="workflow-node-row">
            <span class="workflow-node-number">{{ index + 1 }}</span>
            <el-select v-model="node.nodeType" aria-label="审批人类型">
              <el-option
                v-for="(label, value) in nodeLabels"
                :key="value"
                :label="label"
                :value="value"
              />
            ...
            <el-input
              v-if="node.nodeType === 'SPECIFIC_USER'"
              v-model="node.userId"
              inputmode="numeric"
              placeholder="审批用户 ID"
            />
            <el-input
              v-else-if="node.nodeType === 'HR'"
              v-model="node.roleCode"
              placeholder="HR 角色编码"
            />
            <span v-else class="workflow-node-hint">
              {{
                node.nodeType === 'DIRECT_MANAGER'
                  ? '提交人直属主管'
                  : '提交人所在部门负责人'
              }}
            </span>
