...
const attendance = folder('30 Attendance', [
  ...
  request('API-ATT-02 Create leave type', 'POST', '/leave-types', { token: 'attendanceAdminAccessToken', body: { code: timestampCode('APIFOX_LEAVE'), name: 'APIFOX Leave {{runStamp}}', deductBalance: false, annualQuota: 0, minUnitHours: 1 }, test: capture([`pm.environment.set('leaveTypeId', pm.response.json().data.id);`, `pm.environment.set('leaveTypeVersion', pm.response.json().data.version);`]) }),
  request('API-ATT-03 Update leave type', 'PATCH', '/leave-types/{{leaveTypeId}}', { token: 'attendanceAdminAccessToken', body: { name: 'APIFOX Leave Updated {{runStamp}}', deductBalance: false, annualQuota: 0, minUnitHours: 1, version: '{{leaveTypeVersion}}' }, test: capture([`pm.environment.set('leaveTypeVersion', pm.response.json().data.version);`]) }),
  ...
  request('API-ATT-06 Employee leave balances', 'GET', '/leave-balances/employees/{{employeeId}}', { token: 'attendanceAdminAccessToken' }),
  request('API-ATT-07 Adjust leave balance', 'POST', '/leave-balances/{{testBalanceId}}/adjust', { token: 'attendanceAdminAccessToken', body: { deltaHours: 1, direction: 'INCREASE', reason: 'APIFOX adjustment {{runStamp}}', version: '0' } }),
  request('API-ATT-08 Leave balance changes', 'GET', '/leave-balances/{{testBalanceId}}/changes', { token: 'attendanceAdminAccessToken' }),
  request('API-ATT-09 Create leave request', 'POST', '/leave-requests', { token: 'employeeAccessToken', body: { leaveTypeId: '{{leaveTypeId}}', startTime: '2026-08-10T01:00:00Z', endTime: '2026-08-10T03:00:00Z', reason: 'APIFOX leave {{runStamp}}' }, test: capture([`pm.environment.set('leaveRequestId', pm.response.json().data.id);`]) }),
  ...
  request('API-ATT-13 Create overtime request', 'POST', '/overtime-requests', { token: 'employeeAccessToken', body: { startTime: '2026-08-11T01:00:00Z', endTime: '2026-08-11T03:00:00Z', reason: 'APIFOX overtime {{runStamp}}', compensationType: 'OVERTIME_PAY' }, test: capture([`pm.environment.set('overtimeRequestId', pm.response.json().data.id);`]) }),
  ...
  request('API-ATT-17 Create work calendar', 'POST', '/work-calendars', { token: 'attendanceAdminAccessToken', body: { calendarYear: 2031, name: 'APIFOX Calendar {{runStamp}}', timeZone: 'Asia/Shanghai', status: 'ACTIVE', days: [{ workDate: '2031-01-02', workday: true, workHours: 8, holidayName: null }] }, test: capture([`pm.environment.set('calendarId', pm.response.json().data.id);`, `pm.environment.set('calendarVersion', pm.response.json().data.version);`]) }),
  request('API-ATT-18 Get work calendar', 'GET', '/work-calendars?year=2031', { token: 'attendanceAdminAccessToken' }),
  request('API-ATT-19 Update work calendar', 'PUT', '/work-calendars/{{calendarId}}', { token: 'attendanceAdminAccessToken', body: { name: 'APIFOX Calendar Updated {{runStamp}}', timeZone: 'Asia/Shanghai', status: 'ACTIVE', days: [{ workDate: '2031-01-02', workday: false, workHours: 0, holidayName: 'APIFOX holiday' }], version: '{{calendarVersion}}' } }),
...
const personnel = folder('40 Personnel Changes and Handover', [
  ...
  request('API-PER-05 Submit onboarding change', 'POST', '/personnel-changes/{{personnelChangeId}}/submit', { token: 'hrAccessToken', body: { version: '{{personnelChangeVersion}}' } }),
...
const tasks = folder('50 Workflow Tasks and Completion', [
  ...
  request('API-WF-07 Personnel effective date not reached', 'POST', '/personnel-changes/{{personnelChangeId}}/effective', { token: 'adminAccessToken', body: { version: '1' }, test: expected(409, 'STATE_CONFLICT') }),
